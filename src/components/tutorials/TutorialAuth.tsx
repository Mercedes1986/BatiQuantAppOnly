...
export const TutorialAuth: React.FC<Props> = ({ onUnlock }) => {
  ...
  const VALID_CODE = useMemo(() => 'PRO2024', []);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-slate-900 p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">Espace Tutoriels</h1>
          <p className="text-slate-400 text-sm">Accédez aux guides experts et méthodes de calcul détaillées.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="p-8">
          <div className="mb-6">
            ...
            {isLocked && (
              <div className="mt-3 text-amber-700 text-sm">
                Trop d’essais. Réessayez dans {remainingSec}s.
              </div>
            )}
          </div>
          ...
          >
            Déverrouiller <ArrowRight size={20} className="ml-2" />
          </button>
