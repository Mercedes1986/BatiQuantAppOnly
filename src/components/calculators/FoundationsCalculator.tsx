...
import { CalculatorType, CalculationResult, Unit, FoundationProjectInputs, PadConfig, FoundationType } from '../../../types';
...
import { 
  Warehouse, ArrowRight, Check, AlertTriangle, Settings, Info, 
  Ruler, Shovel, Layers, CircleDollarSign, Plus, Trash2, BoxSelect, Combine,
...
const inputClass = "w-full p-2.5 border border-slate-300 rounded-lg text-slate-900 bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-500 transition-all text-sm font-medium placeholder:text-slate-400";
const labelClass = "block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wide";
const sectionClass = "bg-white p-5 rounded-2xl shadow-sm border border-slate-100 space-y-4 animate-in fade-in slide-in-from-bottom-2";
...
export const FoundationsCalculator: React.FC<Props> = ({ onCalculate, initialMode = 'simple' }) => {
  ...
  const [prices, setPrices] = useState({
    ...
    STEEL_KG: getUnitPrice('REBAR_KG') || 1.80,
    ...
    DRAIN_ML: getUnitPrice('DRAIN_PIPE_50M') / 50 || 1.40,
    GRAVEL_M3: getUnitPrice('GRAVEL_FOUNDATION_TON') * 1.5 || 67.5, // Ton to M3 approx
  });
  ...
  const addPad = () => {
    setPads([...pads, { id: Date.now().toString(), count: 1, lengthCm: 60, widthCm: 60, heightCm: 40, depthCm: parseFloat(excavationDepthCm) || 80 }]);
  };

  const updatePad = (id: string, field: keyof PadConfig, val: number) => {
    setPads(pads.map(p => p.id === id ? { ...p, [field]: val } : p));
  };
  ...
  const results = useMemo(() => {
    const inputs: FoundationProjectInputs = {
      ...
      type,
      totalLengthMl: parseFloat(totalLengthMl) || 0,
      stripWidthCm: parseFloat(stripWidthCm) || 0,
      stripHeightCm: parseFloat(stripHeightCm) || 0,
      pads,
      excavationDepthCm: parseFloat(excavationDepthCm) || 0,
      trenchOverwidthCm: parseFloat(trenchOverwidthCm) || 0,
      soilType,
      frostDepthCm: parseFloat(frostDepthCm) || 0,
      groundwater,
      cleanConcrete,
      cleanConcreteThickCm: parseFloat(cleanConcreteThickCm) || 0,
      formwork,
      drainage,
      drainageGravel,
      evacuateSpoil,
      reuseSpoil,
      steelRatio: parseFloat(steelRatio) || 0
    };

    return calculateFoundations(inputs, prices);
  }, [type, totalLengthMl, stripWidthCm, stripHeightCm, pads, excavationDepthCm, trenchOverwidthCm, soilType, frostDepthCm, groundwater, cleanConcrete, cleanConcreteThickCm, formwork, drainage, drainageGravel, evacuateSpoil, reuseSpoil, steelRatio, prices, proMode]);

  // Pass to parent
  useEffect(() => {
    const total = results.materials.reduce((sum, m) => sum + m.totalPrice, 0);
    onCalculate({
      summary: `${results.volumes.concrete.toFixed(1)} m³ de fondations`,
      details: [
        { label: 'Type', value: type === 'strip' ? 'Filantes' : type === 'pad' ? 'Isolées' : 'Longrines', unit: '' },
        { label: 'Béton', value: results.volumes.concrete.toFixed(1), unit: 'm³' },
        { label: 'Acier', value: results.quantities.steel.toFixed(0), unit: 'kg' },
        { label: 'Terrassement', value: results.volumes.excavation.toFixed(1), unit: 'm³' }
      ],
        ...
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 text-blue-800 text-sm rounded-xl flex items-start border border-blue-100">
            ...
            <div>
              ...
              <p className="opacity-80 mt-1">Calculez le béton, l'acier, le terrassement et l'évacuation en une seule fois.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <button 
              ...
              className={`p-4 rounded-xl border-2 text-left transition-all ${type === 'strip' ? 'border-blue-600 bg-blue-50/50 ring-1 ring-blue-200' : 'border-slate-100 bg-white hover:border-blue-200'}`}
            >
            ...
            <button 
              onClick={() => setType('pad')}
              className={`p-4 rounded-xl border-2 text-left transition-all ${type === 'pad' ? 'border-blue-600 bg-blue-50/50 ring-1 ring-blue-200' : 'border-slate-100 bg-white hover:border-blue-200'}`}
            >
              <div className="flex items-center mb-2">
          ...
          <button onClick={() => setStep(2)} className="w-full py-3.5 bg-blue-600 text-white rounded-xl font-bold flex justify-center items-center shadow-lg shadow-blue-200 active:scale-[0.98] transition-transform">
        ...
        <div className="space-y-4">
          <div className={sectionClass}>
            ...
            {type === 'strip' && (
              ...
              <div>
                ...
                <input type="number" value={totalLengthMl} onChange={e => setTotalLengthMl(e.target.value)} className={inputClass} placeholder="Ex: 45"/>
              </div>
            ...
            {type === 'pad' && (
              <div className="space-y-4">
                {pads.map((pad, idx) => (
                  <div key={pad.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 relative">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      {pads.length > 1 && <button onClick={() => removePad(pad.id)} className="text-red-400 p-1"><Trash2 size={16}/></button>}
                    </div>
                ...
                <button onClick={addPad} className="w-full py-2 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 font-bold flex justify-center items-center hover:bg-slate-50 hover:text-blue-500 hover:border-blue-300 transition-colors">
          ...
          <div className={sectionClass}>
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-1">Espace supplémentaire de chaque côté de la semelle.</p>
            </div>
          </div>

          <div className="flex gap-3 pt-4">
             <button onClick={() => setStep(step-1)} className="p-3 rounded-xl bg-white border border-slate-200 text-slate-500"><ArrowLeft/></button>
             <button onClick={() => setStep(step+1)} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">Suivant</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
           <div className={sectionClass}>
              ...
              <select value={soilType} onChange={e => setSoilType(e.target.value)} className={inputClass}>
                 {SOIL_PROPERTIES.map(s => <option key={s.id} value={s.id}>{s.label} (Foisonnement x{s.bulkingFactor})</option>)}
              </select>
           ...
           <div className={sectionClass}>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <input type="number" value={frostDepthCm} onChange={e => setFrostDepthCm(e.target.value)} className={inputClass} placeholder="0 si inconnu"/>
                 </div>
              ...
              {parseFloat(frostDepthCm) > 0 && parseFloat(excavationDepthCm) < parseFloat(frostDepthCm) && (
        ...
        <div className="space-y-4">
           <div className={sectionClass}>
              ...
              <div>
                 ...
                 <input type="range" min="40" max="150" step="5" value={steelRatio} onChange={e => setSteelRatio(e.target.value)} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/>
              ...
              <div className="bg-slate-50 p-3 rounded-lg text-xs text-slate-600 mt-4">
                 <p className="mb-1"><span className="font-bold">Estimation Pro :</span> Le calcul se base sur un ratio moyen au m³ de béton.</p>
              ...
              <div className="space-y-3">
                 ...
                 <label className="flex items-center justify-between p-3 border rounded-lg hover:bg-slate-50 cursor-pointer">
                    <div>
                       ...
                       <span className="text-xs text-slate-400">Garder une partie pour remblai (~40%)</span>
                    </div>
           ...
           <div className={sectionClass}>
              ...
              <div className="grid grid-cols-2 gap-3">
                 <div>
                    ...
                    <input type="number" value={prices.CONC_M3} onChange={e => setPrices({...prices, CONC_M3: parseFloat(e.target.value)})} className={inputClass}/>
                 </div>
                 <div>
                    <label className={labelClass}>Acier (€/kg)</label>
                    <input type="number" value={prices.STEEL_KG} onChange={e => setPrices({...prices, STEEL_KG: parseFloat(e.target.value)})} className={inputClass}/>
                 </div>
                 <div>
                    <label className={labelClass}>Fouilles (€/m³)</label>
                    <input type="number" value={prices.EXCAV_M3} onChange={e => setPrices({...prices, EXCAV_M3: parseFloat(e.target.value)})} className={inputClass}/>
                 </div>
                 <div>
                    <label className={labelClass}>Évacuation (€/m³)</label>
                    <input type="number" value={prices.WASTE_M3} onChange={e => setPrices({...prices, WASTE_M3: parseFloat(e.target.value)})} className={inputClass}/>
                 </div>
                 {cleanConcrete && (
                    <div>
                       <label className={labelClass}>B. Propreté (€/m³)</label>
                       <input type="number" value={prices.LEANCONC_M3} onChange={e => setPrices({...prices, LEANCONC_M3: parseFloat(e.target.value)})} className={inputClass}/>
                    </div>
                 )}
                 {formwork && (
                    <div>
                       <label className={labelClass}>Coffrage (€/m²)</label>
                       <input type="number" value={prices.FORMWORK_M2} onChange={e => setPrices({...prices, FORMWORK_M2: parseFloat(e.target.value)})} className={inputClass}/>
                    </div>
                 )}
              </div>
           </div>
