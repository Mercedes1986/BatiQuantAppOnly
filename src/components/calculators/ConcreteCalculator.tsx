...
export const ConcreteCalculator: React.FC<Props> = ({
  onCalculate,
  initialArea,
  initialPerimeter,
}) => {
  ...
  // ✅ helper: prix = override local (ici via state prices) > catalogue > defaultPrices > fallback
  ...
  const [prices, setPrices] = useState(() => ({
    ...
    // pro
    laborM2: 28, // MO pose/finition par m² (option)
  }));
  ...
  // --- Calculation Engine (useMemo + effect) ---
  const calculationData = useMemo(() => {
    ...
    if (isBPE) {
      const volOrdered = Math.ceil(volTotal * 2) / 2; // 0.5m3 steps
      ...
      add({
        ...
        category: CalculatorType.CONCRETE,
        details: `Commandé par pas de 0,5m³ (besoin: ${volTotal.toFixed(2)}m³)`,
      });
    ...
    if (useMesh) {
      ...
      const usefulArea = panelArea * 0.85; // overlap/waste
      ...
      add({
        ...
        totalPrice: parseFloat(cost.toFixed(2)),
        category: CalculatorType.CONCRETE,
        details: `≈ ${usefulArea.toFixed(2)}m² utiles/panneau`,
      });
    }
    ...
    if (useFormwork && perimeter > 0) {
      ...
      add({
        ...
        totalPrice: parseFloat(cost.toFixed(2)),
        category: CalculatorType.CONCRETE,
        details: `${perimeter.toFixed(1)}ml × ${hSlab.toFixed(2)}m`,
      });
    }
    ...
    if (proMode) {
      ...
      add({
        id: "labor",
        name: "Main d'œuvre (pose/finition)",
    ...
    details.push({ label: "Volume béton", value: volTotal.toFixed(2), unit: "m³" });

    return {
      totalCost: parseFloat(totalCost.toFixed(2)),
      materials: materialsList,
      details,
      summary: `${volTotal.toFixed(1)} m³ de béton`,
    };
  }, [
    // geometry
    shapeMode,
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="p-3 bg-blue-50 text-blue-800 text-xs rounded-lg flex items-start">
            ...
            Définissez la surface et l’usage de la dalle pour pré-configurer les options.
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-bold text-slate-500 mb-1">
                  Périmètre (m) <span className="text-[10px] font-normal text-slate-400">(optionnel, utile pour coffrage)</span>
                </label>
                <input
                  type="number"
                  value={perimInput}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-3 rounded-lg border border-slate-200 space-y-3">
            <div>
              ...
              <p className="text-[10px] text-slate-400">Couche drainante compactée.</p>
            </div>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="p-3 bg-blue-50 text-blue-800 text-xs rounded-lg flex items-start">
            ...
            Configuration du béton, ferraillage, coffrage et joints.
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full p-2 border rounded font-bold text-slate-900 bg-white"
              />
            </div>
            <div>
          ...
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="flex bg-white rounded border border-slate-200 p-0.5">
                <button type="button" onClick={() => setIsBPE(true)} className={`px-2 py-1 text-xs rounded ${isBPE ? "bg-blue-100 text-blue-700 font-bold" : "text-slate-500"}`}>
                  Toupie
                </button>
                <button type="button" onClick={() => setIsBPE(false)} className={`px-2 py-1 text-xs rounded ${!isBPE ? "bg-blue-100 text-blue-700 font-bold" : "text-slate-500"}`}>
                  Bétonnière
                </button>
              </div>
            </div>
            ...
            {isBPE && (
              <label className="flex items-center mt-3">
                ...
                <span className="text-sm">Ajouter pompe</span>
              </label>
            )}
          </div>
          ...
          <div className="bg-white p-3 rounded-xl border border-slate-200">
            ...
            {isBPE ? (
              <div className="grid grid-cols-2 gap-3 bg-blue-50 p-3 rounded border border-blue-100">
                ...
                <div className="col-span-2">
                  <label className="block text-[10px] uppercase font-bold text-blue-800 mb-1">Pompe (€)</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Sable (€/t)</label>
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Gravier (€/t)</label>
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              <div>
                <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Treillis (€/panneau)</label>
