...
import { LEVELING_PRODUCTS, LEVELING_SUBSTRATES } from "../../constants";
...
/**
 * LEVELING_PRODUCTS expectation (from constants):
 * { id, label, minThick, maxThick, density } where density = kg / (m²·mm)
 * If your constant differs, adapt the mapping in productDef below.
 */
...
export const LevelingCalculator: React.FC<Props> = ({ onCalculate }) => {
  ...
  const [prices, setPrices] = useState({
    compoundBag: getUnitPrice("RAGREAGE_BAG_25KG") || 0,
    compoundFibre: getUnitPrice("RAGREAGE_FIBRE_25KG") || 0,
    primerL: getUnitPrice("PRIMER_FLOOR_LITER") || 0,
    bandM: getUnitPrice("PERIPHERAL_BAND_M") || 0,
    meshRoll: 40.0, // roll ~50m²
    laborM2: 25.0,
    laborPrep: 8.0,
  });
  ...
  const addZone = () => {
    ...
    if (newZoneThicknessMode === "avg") {
      ...
      // Average thickness from min/max (simple average)
  ...
  // --- Auto recommendations (non-destructive to manual product choice) ---
  ...
  useEffect(() => {
    ...
    // Recommend fiber if Wood/Tile or thick > 10mm
    if (hasWood || hasTile) {
      setProductId("fibre");
      // mesh strongly recommended on wood
  ...
  const calculationData = useMemo(() => {
    ...
    const productDef =
      LEVELING_PRODUCTS.find((p: any) => p.id === productId) || (LEVELING_PRODUCTS[0] as any);
    ...
    zones.forEach((z) => {
      ...
      // perimeter estimate: square approximation
      perimeterTotal += Math.sqrt(z.area) * 4;

      const th = z.thicknessVal; // avg mm
      const density = toNum((productDef as any).density, 1.6); // kg/(m2·mm) typical range ~1.5-1.7
      const w = z.area * th * density;
      totalWeight += w;
      ...
      if (z.thicknessMode === "minmax" && z.thicknessMin !== undefined && z.thicknessMax !== undefined) {
        if (z.thicknessMax - z.thicknessMin >= 10) {
          warnings.push(`${z.label}: écart min/max important (${z.thicknessMin}-${z.thicknessMax}mm) → prévoir passes / rattrapage.`);
        }
      }
    ...
    if (usePrimer && totalArea > 0) {
      ...
      materialsList.push({
        ...
        category: CalculatorType.RAGREAGE,
        details: zones.some((z) => z.substrate === "tile") ? "Rénovation / grip" : "Universel",
      });
    }
    ...
    if (proMode && totalArea > 0) {
      ...
      materialsList.push(
        {
          id: "lab_prep",
          name: "Préparation (primaire / ponçage)",
    ...
    const avgThick =
      zones.length > 0 ? zones.reduce((a, b) => a + b.thicknessVal, 0) / zones.length : 0;
              ...
              >
                <div>
                  ...
                  <span className="text-xs text-slate-500">
                    ...
                    • {LEVELING_SUBSTRATES.find((s: any) => s.id === z.substrate)?.label || z.substrate}
                  </span>
                </div>
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded ${
                  newZoneThicknessMode === "avg" ? "bg-white shadow" : "text-slate-500"
                }`}
              >
                Épaisseur moy.
              </button>
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-bold rounded ${
                  newZoneThicknessMode === "minmax" ? "bg-white shadow" : "text-slate-500"
                }`}
              >
                Min / Max
              </button>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="p-3 bg-blue-50 text-blue-800 text-xs rounded-lg flex items-start">
            ...
            Sélection du ragréage. (Auto-reco activée :{" "}
            <span className="font-bold">{autoProductLocked ? "oui" : "non"}</span>)
          </div>
          ...
          <div className="bg-white p-3 rounded-xl border border-slate-200">
            <div className="flex justify-between items-center mb-3">
              ...
              <label className="flex items-center gap-2 text-xs text-slate-600">
                <input
                  type="checkbox"
                  checked={autoProductLocked}
                  onChange={(e) => setAutoProductLocked(e.target.checked)}
                />
                Auto
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <select
                  value={bagSize}
                  onChange={(e) => setBagSize(toNum(e.target.value, 25))}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
              ...
              {usePrimer && (
                <div className="pl-2 flex items-center space-x-3">
                  ...
                  <div className="flex bg-slate-100 rounded p-0.5">
                    <button
                      onClick={() => setPrimerLayers(1)}
                      className={`px-3 py-1 text-xs rounded ${
                        primerLayers === 1 ? "bg-white shadow font-bold" : ""
                      }`}
                    >
                      1
                    </button>
                    <button
                      onClick={() => setPrimerLayers(2)}
                      className={`px-3 py-1 text-xs rounded ${
                        primerLayers === 2 ? "bg-white shadow font-bold" : ""
                      }`}
                    >
                      2
                    </button>
            ...
            <label className="flex items-center justify-between p-3 bg-white border rounded-lg cursor-pointer">
              ...
              <input
                type="checkbox"
                checked={usePeripheralBand}
                onChange={(e) => setUsePeripheralBand(e.target.checked)}
                className="h-5 w-5 text-blue-600 rounded"
              />
            </label>
          ...
          <div className="bg-white p-3 rounded-xl border border-slate-200">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  type="number"
                  value={prices.compoundBag}
                  onChange={(e) => updatePrice("compoundBag", e.target.value)}
              ...
              <div>
                ...
                <input
                  type="number"
                  value={prices.compoundFibre}
                  onChange={(e) => updatePrice("compoundFibre", e.target.value)}
                  className="w-full p-1.5 border rounded text-sm bg-white text-slate-900"
                />
              </div>
              ...
              {useMesh && (
                <div>
                  ...
                  <input
                    type="number"
                    value={prices.meshRoll}
                    onChange={(e) => updatePrice("meshRoll", e.target.value)}
                    className="w-full p-1.5 border rounded text-sm bg-white text-slate-900"
                  />
                </div>
              )}
              ...
              {usePeripheralBand && (
                <div>
                  ...
                  <input
                    type="number"
                    value={prices.bandM}
                    onChange={(e) => updatePrice("bandM", e.target.value)}
                    className="w-full p-1.5 border rounded text-sm bg-white text-slate-900"
                  />
                </div>
              )}
            ...
            {proMode && (
              <div className="mt-4 pt-3 border-t border-slate-100 grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={prices.laborPrep}
                    onChange={(e) => updatePrice("laborPrep", e.target.value)}
                    className="w-full p-1.5 border border-blue-200 rounded text-sm bg-white text-slate-900"
                  />
                </div>
              </div>
