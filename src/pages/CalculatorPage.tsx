...
import { getCalculators, getMaterialImageUrl, getStaticTips } from "../constants";
...
/**
 * Fix goals:
 * - Remove invalid `item.key` usage (MaterialItem has no `key`) -> TS2339 fix.
 * - Robustly find an image key: refKey > imageKey > sku > code > id.
 * - Safe fallback to _missing.png.
 */
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
              ...
              <ul className="space-y-4 text-sm">
                {result.materials.map((m) => {
                  const imageKey = getImageKeyFromItem(m as any);
