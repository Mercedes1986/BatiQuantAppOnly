...
export const SettingsPage: React.FC = () => {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      alert(t("settings.file_too_big", { defaultValue: "File too large (max 5 MB)." }));
      resetFileInput();
      return;
    }
    const ok = confirm(
      t("settings.import_confirm", {
        defaultValue: "Import will replace your current data. Continue?",
      })
    );
    if (!ok) {
      resetFileInput();
    ...
    reader.onerror = () => {
      ...
      resetFileInput();
      alert(t("settings.file_read_error", { defaultValue: "File read error." }));
    };
    reader.onload = (evt) => {
      try {
        const content = evt.target?.result;
        if (!content || typeof content !== "string") {
          alert(t("settings.invalid_backup", { defaultValue: "Invalid backup file." }));
          return;
        }
  ...
  const rowClass =
    "w-full px-4 py-4 flex flex-col gap-3 text-left transition-colors hover:bg-white/70 sm:flex-row sm:items-center sm:justify-between";
      ...
      <div className="page-frame space-y-4">
        <section className="glass-panel rounded-[28px] p-4 sm:rounded-[32px] sm:p-5 md:p-6">
          <div className="flex flex-col gap-4">
            <div>
              <h1 className="text-[24px] font-extrabold leading-tight text-slate-800 sm:text-2xl">
            ...
            <div className="max-w-full overflow-x-auto no-scrollbar">
              <div className="mx-auto flex w-fit items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-200/80 p-1.5 shadow-sm">
                ...
                {tabButton(
                  "company",
                  t("settings.tabs.company", { defaultValue: "Company" }),
                  <Building2 size={16} />
                )}
              </div>
            </div>
        ...
        {activeTab === "company" ? (
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center p-4 sm:p-5">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-extrabold text-slate-800">
                    {t("settings.pro.title", { defaultValue: "Pro" })}
                  </h3>
                  <p className="text-xs text-slate-500">
                    {t("settings.pro.subtitle", { defaultValue: "Unlock advanced features." })}
                  </p>
                </div>
              </div>
            </section>
            ...
            <section className="glass-panel overflow-hidden rounded-[28px]">
              <h3 className="flex items-center px-4 pt-4 text-xs font-extrabold uppercase tracking-wider text-slate-400 sm:px-5 sm:pt-5">
                <HardDrive size={12} className="mr-2" />
                {t("settings.data.title", { defaultValue: "Data" })}
              </h3>
              ...
              <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 sm:p-5">
                ...
                >
                  ...
                  <span className="text-sm font-extrabold text-slate-700">
                    {t("settings.data.backup", { defaultValue: "Backup" })}
                  </span>
                  <span className="text-[10px] text-slate-400">
                    {t("settings.data.export_json", { defaultValue: "Export JSON" })}
                  </span>
                </button>
                ...
                <label
                  className={`group flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white/70 p-5 transition-colors ${
                    isImporting ? "pointer-events-none opacity-60" : "hover:border-emerald-200 hover:bg-emerald-50/70"
                  }`}
                >
                  ...
                  <span className="text-[10px] text-slate-400">
                    {t("settings.data.import_json", { defaultValue: "Import JSON" })}
                  </span>
                  <input
                    type="file"
                    ref={fileInputRef}
                ...
                <div className="flex items-start rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs text-amber-700">
                  ...
                  <p>
                    {t("settings.data.warning", {
                      defaultValue: "Keep a recent backup of your data.",
                    })}
                  </p>
                </div>
              </div>
            ...
            <section className="glass-panel overflow-hidden rounded-[28px]">
              <h3 className="px-4 pt-4 text-xs font-extrabold uppercase tracking-wider text-slate-400 sm:px-5 sm:pt-5">
              ...
              <div className="divide-y divide-slate-100">
                <div className={rowClass}>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600 focus:ring-0 sm:w-auto sm:min-w-[140px]"
                  >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-slate-700">
                        {t("settings.support.help", { defaultValue: "Help & FAQ" })}
                      </span>
                    </div>
            ...
            <div className="pt-2 text-center">
              ...
              <p className="mt-2 text-[10px] text-slate-300">
                {t("settings.footer", { defaultValue: "No personal data is sold." })}
              </p>
            </div>
          </div>
        )}
